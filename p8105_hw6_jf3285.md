...
ordered based most frequent to infrequent. First, a linear model with
all the variables inside is used to see if there’s a good fit, and based
on the summary result, non-significant predictors (large p-value) are
...
strongly associated with mother’s race and baby’s length at birth also
depends on the average number of cigarettes mothers smoked per day
during pregnancy.
